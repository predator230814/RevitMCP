...
public sealed class DescribeParametersBridgeOrchestrationTests
{
    ...
    {
        var discovery = new FakeDiscovery();
        discovery.Instances.Add(TestSupport.Ready("id-v6", "pipe-v6", protocolVersion: 5));
        var factory = Factory((pipe, _, _) =>
        {
            ...
            return Task.FromResult(new RecordingBridgeClient
            {
                ...
                DescribeParameters = (_, _, _) => throw new InvalidOperationException("DescribeParameters must not run for unknown v6.")
            });
        });
