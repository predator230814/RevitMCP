...
public sealed class DescribeParametersRoutingTests
{
    ...
    public void Describe_parameters_eligibility_is_protocol_v5_only()
    {
        ...
        Assert.False(InstanceTargetResolver.IsDescribeParametersEligible(TestSupport.Ready("v6", "pipe-v6", protocolVersion: 6)));
    }
    ...
    public async Task Explicit_v6_id_returns_unavailable()
    {
        ...
        var factory = UnusedFactory();
        var outcome = await CreateService(discovery, factory).ExecuteAsync("v6", TestSupport.CreateDescribeParametersRequest(), CancellationToken.None);
