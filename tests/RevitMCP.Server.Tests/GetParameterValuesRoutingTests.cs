...
public sealed class GetParameterValuesRoutingTests
{
    [Fact]
    public void Get_parameter_values_eligibility_is_protocol_v6_only()
    {
        Assert.False(InstanceTargetResolver.IsGetParameterValuesEligible(TestSupport.Ready("v1", "pipe-v1", protocolVersion: 1)));
        Assert.False(InstanceTargetResolver.IsGetParameterValuesEligible(TestSupport.Ready("v2", "pipe-v2", protocolVersion: 2)));
        Assert.False(InstanceTargetResolver.IsGetParameterValuesEligible(TestSupport.Ready("v3", "pipe-v3", protocolVersion: 3)));
    ...
    [Fact]
    public void Inherited_capability_eligibility_remains_explicit_on_v6()
    {
    ...
    [Fact]
    public async Task Zero_values_capable_instances_returns_no_revit_instance()
    {
    ...
    [Fact]
    public async Task Exactly_one_values_capable_instance_is_auto_selected()
    {
        var discovery = new FakeDiscovery();
        discovery.Instances.Add(TestSupport.Ready("v5", "pipe-v5", protocolVersion: 5));
    ...
    [Fact]
    public async Task Multiple_values_capable_instances_require_instance_id()
    {
        var discovery = new FakeDiscovery();
    ...
    public void Ambiguity_candidates_are_sorted_by_opaque_instance_id()
    {
        var discovered = new[]
        {
            TestSupport.Ready("m", "pipe-m", protocolVersion: 6),
            TestSupport.Ready("a", "pipe-a", protocolVersion: 6),
            TestSupport.Ready("z", "pipe-z", protocolVersion: 6)
        };

        var resolution = InstanceTargetResolver.ResolveForGetParameterValues(discovered, instanceId: null);
    ...
    public async Task Explicit_unknown_id_returns_not_found()
    {
        ...
        discovery.Instances.Add(TestSupport.Ready("alpha", "pipe-alpha", protocolVersion: 6));
        var factory = UnusedFactory();
        var outcome = await CreateService(discovery, factory).ExecuteAsync("missing", TestSupport.CreateGetParameterValuesRequest(), CancellationToken.None);
    ...
    [Fact]
    public async Task Explicit_v5_id_returns_unavailable()
    {
        var discovery = new FakeDiscovery();
        discovery.Instances.Add(TestSupport.Ready("v5", "pipe-v5", protocolVersion: 5));
        discovery.Instances.Add(TestSupport.Ready("v6", "pipe-v6", protocolVersion: 6));
        var factory = UnusedFactory();
        var outcome = await CreateService(discovery, factory).ExecuteAsync("v5", TestSupport.CreateGetParameterValuesRequest(), CancellationToken.None);
    ...
    [Fact]
    public async Task Explicit_v6_id_is_selected()
    {
        var discovery = new FakeDiscovery();
        discovery.Instances.Add(TestSupport.Ready("v5", "pipe-v5", protocolVersion: 5));
        discovery.Instances.Add(TestSupport.Ready("v6", "pipe-v6", protocolVersion: 6));
        var factory = SuccessfulFactory("v6", "pipe-v6");
        var outcome = await CreateService(discovery, factory).ExecuteAsync("v6", TestSupport.CreateGetParameterValuesRequest(), CancellationToken.None);

        Assert.True(outcome.IsSuccess);
        Assert.Equal(new[] { "pipe-v6" }, factory.RequestedPipes);
    }
    ...
    public async Task Explicit_empty_or_whitespace_id_never_auto_selects(string instanceId)
    {
        ...
        discovery.Instances.Add(TestSupport.Ready("only", "pipe-only", protocolVersion: 6));
        var factory = UnusedFactory();
        var outcome = await CreateService(discovery, factory).ExecuteAsync(instanceId, TestSupport.CreateGetParameterValuesRequest(), CancellationToken.None);
    ...
    public async Task Each_invocation_performs_fresh_discovery()
    {
        ...
        await service.ExecuteAsync(null, TestSupport.CreateGetParameterValuesRequest(), CancellationToken.None);
        await service.ExecuteAsync(null, TestSupport.CreateGetParameterValuesRequest(), CancellationToken.None);
        ...
        return new RecordingBridgeClientFactory
        {
            Connect = (pipe, _, _) =>
            {
                ...
                var client = new RecordingBridgeClient
                {
                    ...
                    GetParameterValues = (_, _, _) => Task.FromResult(
                        TestSupport.CreateGetParameterValuesResult(
                            instanceId,
                            "doc-1",
                            TestSupport.CreateParameterValueOk("ref-1", "pref-1")))
                };
